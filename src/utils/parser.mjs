...
export class QuestionParser {
  ...
  */
  constructor(text) {
    ...
    (this.tokens.sort((a, b) => b.length - a.length)).forEach(token => {
  ...
  getTokenId(token) {
    let compId = function (token) {
      ...
      for (let i = 0; i < symbols.length; i++) {
        token = token.replace(symbols[i], '_');
      }
      return token;
    }
    ...
    if (this.values[key] === undefined) {
      ...
      if (obj !== null && obj !== undefined) {
        ...
        for (let i = 0; i < parsed[length]; i++) {
          let j = getRandomInt(0, allvalues.length - 1);
          while (!obj.repeat && used[j] !== undefined) {
            j = getRandomInt(0, allvalues.length - 1);
          }
          used[j] = 1;
